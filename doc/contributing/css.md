...
All class names start with a `.wmre-` namespace to reduce the likelihood of
...
WMRE Design System Frontend uses the Block Element Modifier (BEM) methodology when naming
...
.wmre-card               // Block - the root of a component
.wmre-card__body         // Element - a part of the block
.wmre-card--active       // Modifier - a variant of the block
```
...
```scss
.wmre-phase-banner
.wmre-phase-banner__phase-tag
.wmre-phase-banner__phase-tag--light-blue
```
...
`.wmre-error-summary` modifies the `.wmre-list` component.

Component modifiers use an extra class, scoped to the component:

`.wmre-error-summary__list`
...
The syntax is used to generate a [SassDoc application](http://wmre-frontend-review.herokuapp.com/doc/) that documents SCSS in a readable format.
