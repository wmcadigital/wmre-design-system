...
const componentExampleScript = () => {
  const exampleScripts = document.querySelectorAll('.wmre-website-code-example__js'); // Get all code examples that have the js tab enabled
  ...
  exampleScripts.forEach(exampleScript => {
    ...
    // parentEle.querySelector('.wmre-details--js .js').innerHTML = escapedCharScriptES5;

    // *** END BABEL ES6 => ES5 ***

    parentEle.querySelector('.wmre-details--es6 .js').innerHTML = escapeCharScriptES6; // The find the es6 pre/code block and drop our formatted code in there
  });
};
